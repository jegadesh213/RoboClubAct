...
#define echo A0
#define trig A1
byte maxDist = 100;                               //Maximum sensing distance (Objects further than this distance are ignored)
byte stopDist = 35;                               //Minimum distance from an object to stop in cm
float timeOut = 2*(maxDist+10)/100/340*1000000;   //Maximum time to wait for a return signal

byte motorSpeed = 100;                             //The maximum motor speed
...
int checkDirection()                                            //Check the left and right directions and decide which way to turn
{
  ...
  if (distances[0]>=stopDist && distances[1]>=stopDist){        //If both directions are clear, turn left
    turnDir = 1;
    turnDir = 2;
  }
